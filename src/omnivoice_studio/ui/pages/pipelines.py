...
def create_pipelines_page(state: UIState):
    ...
    async def do_audiobook(
        chapter_files: list[str], speaker: str, lang: str, model_label: str
    ):
        ...
        req = AudiobookRequest(
            chapter_paths=chapter_files, speaker=speaker, language=lang, model=model
        )
    ...
    async def do_script_read(script_text: str, speaker_json: str, model_label: str):
        ...
        req = ScriptReadRequest(
            script_text=script_text, speaker_map=speaker_map, model=model
        )
    ...
    async def do_subtitles(
        srt_file: str, speaker: str, lang: str, model_label: str, preserve: bool
    ):
        if not srt_file:
            return "", None, "No SRT"
        _, m_map = state.model_choices()
    ...
    with gr.Column():
        ...
        with gr.Tab("NPC Pack"):
            ...
            with gr.Row():
                npc_csv = gr.File(
                    label="CSV File (character_id, text, line_id)", file_types=[".csv"]
                )
                npc_json = gr.Textbox(
                    label="Speaker Map (JSON)",
                    lines=5,
                    placeholder='{"char_1": {"type": "custom_voice", "speaker": "Ryan"}}',
                )
        ...
        with gr.Tab("Script Table-read"):
            ...
            with gr.Row():
                scr_text = gr.Textbox(
                    ...
                    lines=10,
                    placeholder="[Ryan] Hello.\n[Vivian] Hi there!",
                )
                scr_json = gr.Textbox(
                    label="Speaker Config (JSON)",
                    lines=5,
                    placeholder='{"Ryan": {"type": "custom_voice", "speaker": "Ryan"}}',
                )
        ...
        with gr.Tab("Subtitle-to-Speech"):
            gr.Markdown(
                "Convert an SRT file into an audio track synchronized with the subtitles."
            )
            with gr.Row():
                sub_file = gr.File(label="SRT File", file_types=[".srt"])
                with gr.Column():
                    sub_spk = gr.Dropdown(
                        choices=CUSTOMVOICE_SPEAKERS, value="Ryan", label="Voice"
                    )
                    sub_lang = gr.Dropdown(
                        choices=LANG_CHOICES, value="English", label="Language"
                    )
                    sub_preserve = gr.Checkbox(
                        label="Preserve Original Timing (Padded Silence)", value=True
                    )
        ...
        async def refresh():
            ...
            cv_labels = [
                label
                for label in labels
                if "customvoice" in label.lower() or "base" in label.lower()
            ]
            return (
                gr.update(choices=cv_labels, value=cv_labels[0] if cv_labels else None),
                gr.update(choices=cv_labels, value=cv_labels[0] if cv_labels else None),
                gr.update(choices=cv_labels, value=cv_labels[0] if cv_labels else None),
                gr.update(choices=cv_labels, value=cv_labels[0] if cv_labels else None),
                gr.update(choices=cv_labels, value=cv_labels[0] if cv_labels else None),
        ...
        lf_btn.click(
            fn=do_long_form,
            inputs=[lf_text, lf_spk, lf_lang, lf_model],
            outputs=[lf_id, lf_audio, lf_meta],
        )
        npc_btn.click(
            fn=do_npc_pack,
            inputs=[npc_csv, npc_json, npc_model],
            outputs=[npc_id, npc_meta],
        )
        ab_btn.click(
            fn=do_audiobook,
            inputs=[ab_files, ab_spk, ab_lang, ab_model],
            outputs=[ab_id, ab_audio, ab_meta],
        )
        scr_btn.click(
            fn=do_script_read,
            inputs=[scr_text, scr_json, scr_model],
            outputs=[scr_id, scr_audio, scr_meta],
        )
        sub_btn.click(
            fn=do_subtitles,
            inputs=[sub_file, sub_spk, sub_lang, sub_model, sub_preserve],
            outputs=[sub_id, sub_audio, sub_meta],
        )

        return refresh, [lf_model, npc_model, ab_model, scr_model, sub_model]
