...
def create_tokenizer_page(state: UIState):
    ...
    async def do_decode(
        codes_path: str, model_label: str
    ...
    with gr.Column():
        ...
        with gr.Row():
            ...
            with gr.Column():
                gr.Markdown("#### Decode")
                d_codes = gr.Textbox(
                    label="Codes JSON Path",
                    placeholder="Path to a .json file containing codes",
                )
        ...
        async def refresh():
            ...
            return (
                gr.update(
                    choices=tok_labels, value=tok_labels[0] if tok_labels else None
                ),
                gr.update(
                    choices=tok_labels, value=tok_labels[0] if tok_labels else None
                ),
        ...
        d_btn.click(
            fn=do_decode, inputs=[d_codes, d_model], outputs=[d_run_id, d_audio, d_meta]
        )
        ...
        async def do_compare(audio_path: str, model_label: str):
            ...
            try:
                ...
                return (
                    audio_path,
                    recon_audio,
                    f"Successfully reconstructed via {model_label}",
                )
            except Exception:
        ...
        c_btn.click(
            fn=do_compare,
            inputs=[c_audio, c_model],
            outputs=[c_out_orig, c_out_recon, c_stats],
        )

        return refresh, [e_model, d_model, c_model]
