...
def create_voices_page(state: UIState):
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            v_type = gr.Dropdown(
                label="Type", choices=["customvoice", "clone", "design_template"]
            )
    ...
    def save_profile(
        ...
        p = VoiceProfile(
            ...
            clone=(
                CloneConfig(
                    ...
                    x_vector_only_mode=xvector,
                )
                if vtype == "clone"
                else None
            ),
            design_template=(
                DesignTemplateConfig(instruct_template=template, example_text=example)
                if vtype == "design_template"
                else None
            ),
        ...
        if res.get("ok"):
            return "Saved successfully!", gr.update(
                choices=[v["id"] for v in state.get_voices()]
            )
