...
def _resolve_short_config(
    ...
    if len(matches) > 1:
        # Multiple modules may expose the same short key.  If every match
        # resolves to the same value, the shorthand is semantically
        # unambiguous; retain the first path for provenance instead of
        # failing merely because an unrelated module is installed.
